<div class="col-md-3">
    <div id="sidebar">
        <div class="sb-heading">
            <h3 class="title text-uppercase">Thông tin bảng báo giá</h3>
            <a href="javascript:;" class="toggle-content" references="#sidebar .sb-content" display="1">
                <i class="fa fa-angle-down"></i>
            </a>
        </div>

        <div class="sb-content">
            <div class="form-group">
                <small>Tiêu đề bảng báo giá</small>
                <input class="form-control input-sm" type="text" />
            </div>

            <div class="form-group">
                <small>Ngày báo giá</small>
                <input class="form-control input-sm" type="date" />
            </div>

            <div class="form-group">
                <small>Nhân viên báo giá</small>
                <input type="text" class="form-control input-sm" />
            </div>

            <div class="form-group">
                <small class="center-block">Khách hàng</small>
                <select name="" id="" class="form-control input-sm pull-left" style="width: 300px">
                    <option value="">Vui lòng chọn khách hàng</option>
                </select>
                <input type="text" class="form-control input-sm text-right" placeholder="Tổng nợ cũ" style="width: 115px" />
            </div>

            <div class="form-group">
                <small>Địa chỉ</small>
                <input type="text" class="form-control input-sm" />
            </div>

            <div class="form-group">
                <small>Kho hàng</small>
                <select name="" id="" class="form-control input-sm">
                    <option value="">Chọn kho hàng</option>
                </select>
            </div>

            <div class="form-group">
                <small>Nhân viên kho</small>
                <input type="text" class="form-control input-sm" value="Phạm Kỳ Khôi" disabled="true" />
            </div>

            <div class="form-group">
                <small>Ghi chú</small>
                <textarea name="" id="" rows="3" class="form-control input-sm"></textarea>
            </div>

            <div class="form-group">
                <label for="" class="lineside text-center">
                    <span class="text">Thành tiền</span>
                </label>
                <ul class="payment">
                    <li>
                        <strong>Tổng tiền: </strong>
                        <span>5,000,000</span>
                    </li>
                    <li>
                        <strong>Chiết khấu (nếu có): </strong>
                        <span><input type="text" class="form-control input-sm text-right" value="10"/></span>
                    </li>
                    <li>
                        <strong>Phải trả: </strong>
                        <span>4,500,000</span>
                    </li>
                </ul>
            </div>
        </div>
        <div class="sb-footer"></div>
    </div>
</div>